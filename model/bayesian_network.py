...
def build_and_train_bayesian_network(train_df):
    ...
    # focuses on dependence among red flags.
    model_columns = list(model.nodes())
    train_model_df = train_df[model_columns].copy()

    # BayesianEstimator smooths probabilities, which helps avoid zero-probability
    # problems for rare red flag combinations.
    model.fit(
        train_model_df,
